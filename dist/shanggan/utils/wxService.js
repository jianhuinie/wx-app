...
/***/ (function(module, exports, __webpack_require__) {
	    ...
	    for (; i < length; i++) {
	        ...
	        if (options) {
	            for (name in options) {
	                ...
	                if (deep && copy && ((typeof copy === 'undefined' ? 'undefined' : (0, _typeof3.default)(copy)) === 'object' || isArray(copy))) {
	                    if (!isArray(copy)) {
	                        clone = src || {};
	                    } else {
	                        clone = src || [];
	...
	var WXMICROSESSION = 'WXMICROSESSION';
	var wxService = {
	    /**
	     * 错误处理弹窗
	     * options 参照官方文档
	     */
	    showErrorDialog: function showErrorDialog(content) {
	        wx.showModal({
	            title: '提示',
	            showCancel: false,
	            content: content
	     ...
	     */
	    sendWxRequest: function sendWxRequest(options) {
